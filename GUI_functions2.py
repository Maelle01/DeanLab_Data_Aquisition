...
class GUI_functions:
    ...
    def __init__(self, dic_instr):
        ...
        self.instr_types =  ['--None--','Keithley 2400','Keithley 2450', 'Yoko', 'SR830', 'SR860','iTC Teslatron','iPS Teslatron','Rotator','Janis Temperature','iTC 3He','Janis Magnet','SIM','tester']
...
#        self.port_inloop.setDisabled(True)
        ...
        self.sweep_back = QCheckBox("Sweep Back?")
        self.sweep_back.toggled.connect(self.sweepback_toggle)
        self.sweep_back.setChecked(True)
        ...
        #Adding Diagonal Plot option
        self.diagonalPlot = QCheckBox('Pair instr')
        self.comboPairedInstr = QComboBox()
        self.comboPairedInstr.addItems(self.dic_instr)
        self.comboPairedInstr.setDisabled(True)
        self.diagonalPlot.toggled.connect(self.comboPairedInstr.setEnabled)
        self.LabelFunctPair = QLabel('Funtion')
        self.ComboFunctPair = QLineEdit(loop_Frame)
        self.ComboFunctPair.setDisabled(True)
        self.diagonalPlot.toggled.connect(self.ComboFunctPair.setEnabled)
...
#        layout.addWidget(self.port_inloop,0,3,1,1)
        ...
        #Add Diagonal Plot option
        layout.addWidget(self.diagonalPlot, 10, 0, 1, 1)
        layout.addWidget(self.comboPairedInstr, 10, 1, 1, 1)
        layout.addWidget(self.LabelFunctPair, 11, 0, 1, 1)
        layout.addWidget(self.ComboFunctPair, 11, 1, 1, 1)
        layout.addWidget(self.sweep_back, 12, 1, 1, 1)
...
#        self.port_midloop.setDisabled(True)
        ...
        self.sweep_between = QCheckBox("Sweep Between Points")
        self.sweep_between_points = QLineEdit(loop_Frame)
        self.sweep_between_points.setDisabled(True)
        self.sweep_between.toggled.connect(self.sweep_between_points.setEnabled)
...
#        layout.addWidget(self.port_midloop,0,3,1,1)
        ...
        layout.addWidget(self.sweep_between, 8, 0, 1, 1)
        layout.addWidget(self.sweep_between_points, 8, 1, 1, 1)        
        layout.addWidget(self.custum_midloop, 9, 0, 1, 1)
        layout.addWidget(self.function_midloop, 9, 1, 1, 1)
        layout.addWidget(self.list_midloop, 10, 0, 1, 1)
        layout.addWidget(self.listfunc_midloop, 10, 1, 1, 1)
        #layout.setRowStretch(j+3, 2)
        loop_Frame.setLayout(layout)
...
#            self.port1=int(self.port_inloop.text())
            ...
            #Add Paired Instr
            if self.diagonalPlot.isChecked() == True:
                self.instr1_paired = str(self.comboPairedInstr.currentText())
                self.parse_custom(self.ComboFunctPair.text())
                self.X1_paired = self.fonction(self.func,self.X1_immune) 
            else:
                self.X1_paired = np.zeros(self.step1)    
            
            self.X2 = None
            self.X3 = None
            self.X1_rev = self.X1[::-1]
            self.X1paired_rev = self.X1_paired[::-1]
            self.X2_rev = None
            self.X3_rev = None
            ...
            self.X1paired_plot = np.zeros((self.repeat1,self.step1))
            self.X2_plot = None
            self.X3_plot = None
            for i in range(self.repeat1):
                    for j in range(self.step1):
                        if self.dir1 == 'Same direction':
                            ...
                            self.X1paired_plot[i][j] = self.X1_paired[j]
                        else:
                            if i % 2 == 0:
                                self.X1_plot[i][j] = self.X1[j]
                                self.X1paired_plot[i][j] = self.X1_paired[j]
                                ...
                                self.X1paired_plot[i][j] = self.X1paired_rev[j]
        if self.dim_meas == '2D':
            self.instr1 = str(self.instr_inloop.currentText())
#            self.port1 = int(self.port_inloop.text())
...
#            self.port3 = int(self.port_outloop.text())
    ...
    def parse_custom(self,string):
        if '+' in string:
            ...
            if '-' in string_split[0]:
                string_split1 = string_split[0].split('-')[1]
                char = string_split1.split('X')[0]
                self.func[0] = -float(char) if char!='X' else -1  
            else:
                char = string_split[0].split('X')[0]
                self.func[0] = float(char) if char !='X' else 1
            self.func[1] = float(string_split[1]) 
        elif '-' in string:
            string_split = string.split('-')
            if len(string_split) == 3:
                char = string_split[1].split('X')[0]
                self.func[0] = -float(char) if char!='X' else -1  
            ...
            self.func[0] = float(string.split('X')[0])
    ...
    def sweepback_toggle(self):
        self.sweepback=(not self.sweepback)