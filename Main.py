...
from Instruments_Janis import Instruments
import qcodes as qc
...
class Main_Oxford2(QDialog):
    def __init__(self, parent=None):
        ...
        mainLayout.addWidget(self.BottomRightGroupBox, 0, 3, 1, 1)
        mainLayout.setColumnStretch(1,3)
        self.setLayout(mainLayout)

        self.setWindowTitle("DeanLab - easure") #Title of the GUI window
    ...
    def updater(self):#Function where plot, and saving of file happens
        ...
        if GUI.dim_meas == '2D':
            ...
            if self.count1 == GUI.step1:#Move to next line
                self.count1 = 0
                self.loop1+=1
                if self.loop1 == GUI.repeat1: #If several repeats of inner loop
                    self.loop1 = 0
                    self.count2+=1
                    sweep_instr1.sweep_val(GUI.start1,GUI.pointdown1,.1)
                    if self.count2 == GUI.step2: #1 Repeat of outer loop done. Map finished if one repeat of outer loop
                        self.count2 = 0
                        self.loop2+=1
        ...
        if self.Combo_l1.currentIndex()==0:
            xdata=np.linspace(self.plotfrom,self.count,num=int(self.count-self.plotfrom))
        else:
